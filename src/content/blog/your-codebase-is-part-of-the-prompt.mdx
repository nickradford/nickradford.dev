...
You ask for one little change.

The agent nods along, burns a bunch of tokens, confidently edits the wrong file, updates a component you did not ask it to touch, and then explains its work like it just solved cold fusion.

Cool. Great. Very helpful.

Sometimes the agent is just wrong. That happens. These tools are useful, but they are still weird little statistical goblins with terminal access.
...
I’ve only recently started reading the original Domain-Driven Design book by Eric Evans, so I’m not coming at this as a DDD expert. I’m not here to cosplay as an enterprise architect or convince you to name a class `UserEntityFactoryServiceProviderImplementation`.
...
That language should show up everywhere.

In conversation. In code. In tests. In docs. In tickets. In function names. In database tables. In the UI when it makes sense.
...
Now there’s another reader in the room.
...
The files it opens. The filenames it greps. The types it sees. The components near the one you mentioned. The tests. The comments. The routes. The database schema. The docs you forgot about. The weird old helper function named `processData` that everyone is afraid to delete.

All of it is context.

All of it is telling the agent what kind of system this is and what words mean inside it.
...
If the codebase uses `email`, `thread`, `message`, and `conversation` interchangeably, now the agent has to wander around like it lost its keys.

It might search for `conversation` and find almost nothing.

Then search for `thread`.

Then find a file called `email-polling.ts`.

Then inspect a `MessageList` component.

Then modify the wrong interval because, honestly, what the hell is the difference between all these nouns?
...
This stuff is annoying for humans.

It is brutal for agents.
...
A vague name gives the agent room to improvise.

And if there is one thing I would like my coding agent to do less of, it is improvising in production-shaped code.
...
`domain.md` is scaffolding.

It helps you notice the drift. It helps you write down the current mess. It helps you decide what the canonical terms should be.
...
Make the agent look first. Make it show you the map. Then decide what to do.
...
Rename one folder. Rename one type. Update the tests. Fix the route names when it’s safe. Add a note to your agent instructions. Use the canonical term in your next prompt.
...
You should still be specific. You should still give context. You should still say what files matter if you know. You should still define the desired behavior instead of only saying what is broken.
...
The second one is longer, but it is not fluff.

It names the concept. It anchors the concept. It tells the agent what not to confuse it with.

That is not enterprise ceremony. That is just being clear.

## Finally

Agentic coding does not remove the need for software design.

It makes design more visible.
...
Sometimes you end up typing “bro wtf are you thinking” into a chat box while staring at a diff that touched the wrong files.
...
But it gives you a way to make your intent easier to understand.

For humans. For agents. For future you at 11:43 PM trying to remember why `thread` and `conversation` are both in the same folder.
